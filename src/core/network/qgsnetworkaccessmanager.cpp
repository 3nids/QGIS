...
void QgsNetworkAccessManager::setupDefaultProxyAndCache( Qt::ConnectionType connectionType )
{
  ...
  if ( proxyEnabled )
  {
    ...
    noProxyURLs = QgsNetworkAccessManager::settingsProxyNoProxyUrls.setValue();
    ...
    const QString proxyTypeString = QgsNetworkAccessManager::settingsProxyProxyType.setValue();
    ...
    const QString authcfg = QgsNetworkAccessManager::settingsProxyAuthcfg.setValue();
    if ( !authcfg.isEmpty( ) )
    {
