...
void QgsDirectoryItem::setMonitoring( Qgis::BrowserDirectoryMonitoring monitoring )
{
  ...
  QgsSettings settings;
  QStringList noMonitorDirs = QgsSettingsRegistryCore::settingsDisableMonitorItemUris.value();
  QStringList alwaysMonitorDirs = QgsSettingsRegistryCore::settingsAlwaysMonitorItemUris.value();
...
{
  const QgsSettings settings;
  if ( QgsSettingsRegistryCore::settingsDisableMonitorItemUris.value().contains( path ) )
    return Qgis::BrowserDirectoryMonitoring::NeverMonitor;
  else if ( QgsSettingsRegistryCore::settingsAlwaysMonitorItemUris.value().contains( path ) )
